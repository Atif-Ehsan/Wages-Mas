...
public class CustomerHome extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Objects.requireNonNull(getSupportActionBar()).setTitle("DAILY WAGES");

        user = FirebaseAuth.getInstance().getCurrentUser();

        bottomNavigationView = findViewById(R.id.bottom_navigation_customer_home);
        FragmentManager fm = getSupportFragmentManager();
        FragmentTransaction ft = fm.beginTransaction();
        ft.add(R.id.frame_layout_Customer, new CustomerHomeFragment());
        ft.commit();
        ...
        bottomNavigationView.setOnItemSelectedListener(item -> {
            if(item.getItemId() == R.id.home) {
                ...
                ftc.replace(R.id.frame_layout_Customer, new CustomerHomeFragment());
                ftc.commit();
                return true;
            }
            if (item.getItemId() == R.id.profile){
                ...
                ftw.replace(R.id.frame_layout_Customer, new CustomerProfileFragment());
                ftw.commit();
                return true;
            }

            if(item.getItemId()==R.id.chatt){
                FragmentManager fmw = getSupportFragmentManager();
                FragmentTransaction ftw = fmw.beginTransaction();
                ftw.replace(R.id.frame_layout_Customer, new ChatFragment());
                ftw.commit();
                return true;
            }
            return false;
        });
//        upadteToken(FirebaseMessaging.getInstance().getToken().toString());
    }
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        ...
        if(id == R.id.refresh){
            ...
            startActivity(new Intent(this, UpdateCustomerProfileActivity.class));
        }
    ...
    private void upadteToken(String token){
